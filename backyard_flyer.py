import time
import datetime
from enum import Enum
import numpy as np
from udacidrone import Drone
from udacidrone.connection import MavlinkConnection
from udacidrone.messaging import MsgID


class Phases(Enum):
    MANUAL = 0
    ARMING = 1
    CHECKPOINT_TRACKING = 2
    LANDING = 3
    DISARMING = 4


class BackyardFlyer(Drone):

    def __init__(self, connection):
        super().__init__(connection)
        self.target_position = np.array([0.0, 0.0, 0.0])
        self.in_mission = True

        # initial state
        self.flight_phase = Phases.MANUAL
        self.waypoints = []
        self.previous = None

        # register all your callbacks here
        self.register_callback(MsgID.LOCAL_POSITION,
                               self.local_position_callback)
        self.register_callback(MsgID.STATE,
                               self.state_callback)

    def add_waypoint(self, pos):
        self.waypoints.append(pos)

    def target_waypoint(self):
        return self.waypoints[0]

    def arrived_at_waypoint(self):
        self.waypoints.pop(0)
        if len(self.waypoints) == 0:
            self.landing_transition()
        else:
            self.start_new_waypoint()

    def start_new_waypoint(self):
        self.cmd_position(*self.target_waypoint(), 0)

    def local_position_callback(self):
        if self.flight_phase == Phases.CHECKPOINT_TRACKING:
            vector_to_target = self.vector_to_target()
            dist_to_target = sum([pos**2 for pos in vector_to_target])
            print(vector_to_target)
            if dist_to_target < 0.3:
                self.arrived_at_waypoint()
        if self.flight_phase == Phases.LANDING:
            print(self.local_position[2])
            if self.local_position[2] > -0.1:
                self.flight_phase = Phases.DISARMING

    def vector_to_target(self):
        x, y, z = self.local_position
        return [target - local for target, local in zip(self.target_waypoint(), [x, y, -z])]

    def state_callback(self):
        if not self.in_mission:
            return
        if self.flight_phase == Phases.MANUAL:
            self.arming_transition()
        elif self.flight_phase == Phases.ARMING:
            if self.armed:
                self.flight_phase = Phases.CHECKPOINT_TRACKING
                self.start_new_waypoint()
        elif self.flight_phase == Phases.DISARMING:
            if not self.armed:
                self.manual_transition()
            else:
                self.disarming_transition()

    def arming_transition(self):
        print("arming transition")
        self.take_control()
        self.arm()

        # set the current location to be the home position
        self.set_home_position(self.global_position[0],
                               self.global_position[1],
                               self.global_position[2])

        self.flight_phase = Phases.ARMING

    def takeoff_transition(self):
        print("takeoff transition")
        target_altitude = 3.0
        self.target_position[2] = target_altitude
        self.takeoff(target_altitude)
        self.flight_phase = Phases.TAKEOFF

    def landing_transition(self):
        print("landing transition")
        self.land()
        self.flight_phase = Phases.LANDING

    def disarming_transition(self):
        print("disarm transition")
        self.disarm()
        self.flight_phase = Phases.DISARMING

    def manual_transition(self):
        print("manual transition")
        self.release_control()
        self.stop()
        self.in_mission = False
        self.flight_phase = Phases.MANUAL

    def start(self):
        self.start_log("Logs", "NavLog.txt")
        print("starting connection")
        super().start()
        self.stop_log()


if __name__ == "__main__":
    conn = MavlinkConnection('tcp:127.0.0.1:5760',
                             threaded=False,
                             PX4=False)
    drone = AssyncFlyer(conn)
    drone.add_waypoint((0, 0, 5))
    drone.add_waypoint((10, 0, 5))
    drone.add_waypoint((10, 10, 5))
    drone.add_waypoint((0, 10, 5))
    drone.add_waypoint((0, 0, 5))
    time.sleep(2)
    drone.start()
